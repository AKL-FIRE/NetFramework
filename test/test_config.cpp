...
sylar::ConfigVar<std::unordered_map<std::string, int>>::ptr g_str_int_unordered_map_value_config =
	sylar::Config::Lookup("system.str_int_unordered_map", std::unordered_map<std::string, int> {{"k", 2}}, "system string int unordered_map");
...
{                              \
  ...
  for(auto& e : v) { \
    SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << #prefix " "#name": " << e; \
  } \
...
{                              \
  auto& v = (g_var)->getValue(); \
  for(auto& e : v) { \
    SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << #prefix " "#name": {" << e.first << " - " << e.second << "}"; \
  } \
  SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << #prefix " "#name" yaml: " << g_var->toString(); \
}
...
sylar::ConfigVar<std::map<std::string, Person>>::ptr g_person_map =
	sylar::Config::Lookup("class.person_map", std::map<std::string, Person>(), "system person");
